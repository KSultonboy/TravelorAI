...
export default function AgencyResetForm({ email, code }: { email: string; code: string }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const res = await fetch("/api/agency-proxy/agency/auth/reset-password", {
        method: "POST",
  ...
  return (
    <div style={wrap}>
      <div style={card}>
        <p style={{ margin: 0, fontSize: 12, letterSpacing: 1, textTransform: "uppercase", color: GREEN, fontWeight: 700 }}>TravelorAI — Agentlik</p>
