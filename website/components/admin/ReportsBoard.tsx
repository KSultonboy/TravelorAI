...
export default function ReportsBoard() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    setError("");
    const [statsResult, toursResult, leadsResult, agenciesResult, healthResult] = await Promise.all([
      adminApi<Stats>("/stats"),
      adminApi<{ items: { approvalStatus?: string }[] }>("/tours?status=all"),
      adminApi<{ items: { status?: string }[] }>("/bookings?status=all"),
  ...
  return (
    <>
      <header className="admin-head-v2">
        <div>
          ...
          <p className="admin-muted">
            Platforma holati bir qarashda.{updatedAt ? ` Yangilangan: ${updatedAt}` : ""}
          </p>
        </div>
      ...
      <section className="admin-stat-row">
        ...
        <div className="admin-stat-v2">
          <Clock3 size={16} />
          <b>{tourCounts.pending_review || 0}</b>
      ...
      <section className="admin-stat-row">
        <div className={`admin-stat-v2${leadCounts.pending ? " admin-stat-v2--warn" : ""}`}>
          <Clock3 size={16} />
          <b>{leadCounts.pending || 0}</b>
